...
NTFY_TOPIC=factory-a7k3m9q2x # push notification topic — auto-generated by agent-setup
NTFY_SERVER=https://ntfy.sh  # ntfy server (leave as-is for hosted, or self-host)
SLACK_WEBHOOK_URL=https://hooks.slack.com/...  # optional Slack channel notifications
```
...
API_SECRET=...                # auto-generated by agent-setup; bearer token for orchestrator writes
```

**Security model:** The orchestrator port is bound to `127.0.0.1` (localhost only — no LAN exposure). All write endpoints require an `Authorization: Bearer <API_SECRET>` header. Secrets are stored encrypted in Vault (auto-initialized on first `make up`) rather than plaintext JSON. Vault root key and unseal key are backed up to macOS Keychain with `make vault-export-keys`.
...
The factory sends push notifications to your phone or desktop for key events. It uses [ntfy.sh](https://ntfy.sh) — a free, open-source pub/sub service with iOS, Android, desktop, and browser apps.

**Setup (automatic):** `make agent-setup` auto-generates a private topic like `factory-a7k3m9q2x4b8nf` (14 random characters — effectively unguessable) and stores it in your macOS Keychain. The subscribe URL is printed at setup time.

**Change or view your topic:** Open `Settings → Authentication` → ntfy Push Notifications. The topic and subscribe URL are shown with one-click copy buttons. Click **Generate** to rotate to a new topic, or **Send test notification** to verify delivery.

**Events that trigger notifications:**

| Event | Priority |
|-------|----------|
| WO needs human review (sign-off checkpoint) | 🔴 High |
| Agent gave up / errored on a WO | 🔴 High |
| WO merged / complete | 🟡 Default |
| Dependabot PR merged | 🟢 Low |
| Dependabot merge conflict auto-rebased | 🟢 Low |

**Security:** Topics on ntfy.sh are public — anyone who knows the URL can subscribe. The auto-generated topic name makes brute-force guessing impractical. For full privacy, point `NTFY_SERVER` to a self-hosted ntfy instance (`docker run -p 8098:80 binwiederhier/ntfy serve`).

**Slack:** Set `SLACK_WEBHOOK_URL` in `Settings → Authentication` for the same events in a Slack channel. Both channels fire in parallel if configured.
