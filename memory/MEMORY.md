...
## Known Invariants
- [git-https-auth-requires-basic-not-bearer](auto_fix-orchestrator-git-fetch-auth-uses-basic-scheme-.md) — GitHub's smart-HTTP git transport requires Basic auth scheme (base64 "x-access-token:<PAT>"), not Bearer, or fetches silently fail
