...
The verdict is parsed by scanning the review text from the bottom up:
...
Scanning from the bottom prevents false positives when the words "LGTM" or "Needs attention" appear in code snippets or discussion text earlier in the review. The first matching line from the bottom is authoritative.

### Exit code semantics

- `0` — LGTM or Needs attention. The workflow step succeeds. A comment is still posted.
- `1` — Review required. The workflow step fails (`continue-on-error: true` allows downstream steps to run). The final gate step (`if: steps.claude_review.outcome == 'failure'`) then runs `exit 1` without `continue-on-error`, which is what actually fails the required status check in GitHub.
