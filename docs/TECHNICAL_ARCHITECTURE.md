...
**SECONDARY_REPOS:** Set `SECONDARY_REPOS=owner/repo:path/to/wos` (comma-separated for multiple) to have the orchestrator poll additional GitHub repositories for WO specs. Secondary specs are merged into the board view and into the PM assistant's context (`_wo_status_summary()`), but they are **not dispatchable** — the dispatch queue, claim/dispatch loop, and PR-watchdog only ever operate on the primary repo (`GITHUB_REPO`). A user who wants the factory to *autonomously build* two separate products needs two factory instances (two compose stacks, two `GITHUB_REPO` values) — `SECONDARY_REPOS` only buys board visibility for a repo's WOs, not automated agent dispatch against them.

**WO number reservation is repo-scoped** (WO-1052, fixed 2026-07-30): `POST /api/wos/reserve` accepts optional `repo`/`wo_path`, defaulting to `GITHUB_REPO`/`WO_PATH`. Reservations are tracked per repo (`reserved_wos.json` is keyed by repo), so two repos with overlapping WO-number ranges — e.g. this repo's own `docs/work_orders/` and a product repo's `docs/project_management/work_orders/`, both numbering from ~1000 — don't collide. Before this fix, the endpoint had no repo concept at all and always numbered against the primary repo regardless of which repo the caller actually meant.
...
**Model config endpoints** (2026-07-30): `GET/PUT /api/settings/automation-model` and `GET/PUT /api/settings/review-model` both persist to GitHub repo variables (`ANTHROPIC_MODEL`, `ANTHROPIC_MODEL_REVIEW`) on `GITHUB_REPO` — a repo variable, not a local secret/env var, because that's the only thing readable by both this dashboard and the GitHub-Actions-run scripts, which have no network path back to this machine (orchestrator is bound to `127.0.0.1`). Automation Model is cached in-memory (`_get_model()`), refreshed every 10 minutes plus on every PUT, since local callers (WO drafting, PM chat) need a fast read. Review Model has no such cache — nothing in this process consumes it, it's a pure pass-through for the dashboard, so a live GitHub API read per request is fine. `POST /api/internal/anthropic-key` resolves the Anthropic key (env-first, then Vault) for other in-stack services — `agent-runner` specifically, which can't read this process's env/Vault directly — gated behind POST rather than GET since the bearer-auth middleware only guards non-GET requests and a raw key must never sit behind an unauthenticated route.

**GITHUB_TOKEN is live-resolvable, GITHUB_REPO is not** (fixed 2026-07-30): `_get_github_token()` checks env first, then Vault, re-read on every call — rotating the token via Settings → Authentication takes effect immediately, no restart. `GITHUB_REPO` can't get the same treatment: it's baked into dozens of function default-parameter values throughout this file, evaluated once at import. Changing which repo this factory instance targets is a restart, by design, not an oversight.
...
**Diff truncation.** All scripts truncate diffs at 2,000–4,000 lines. Very large PRs receive partial reviews. The AI review comment notes when truncation occurred.

**PR size is enforced, but only after the fact.** `quality_gate.py`'s PR size gate (`_MAX_FILES_CHANGED = 30`, `_MAX_LINES_CHANGED = 800`) blocks a WO from reaching human review once its diff exceeds those limits — but nothing checks a WO's likely scope before dispatch. A single legitimately-scoped WO can exceed the gate on its own (WO-433, ~950 lines, hit this directly), at which point the only path forward is a human finishing the PR by hand or the WO spec being split — the automated pipeline has no way to shrink an already-written diff itself. If a WO's `## What to Build` section reads as multi-part, consider splitting it into separate WOs at spec time rather than relying on the gate to catch it after an agent has already implemented it as one piece.

**The runtime factory and the developer's desktop are typically the same machine.** In a single-workstation deployment, every dispatch entry's `workstation` field is that one Mac — one Docker daemon, one physical set of containers that agent quality-gates rebuild live. A `launchctl kickstart -k` on one runner process, or the machine sleeping, can silently take down shared infrastructure (e.g. a CI lock file, see below) until a human notices. This is a legitimate, cheap way to run the factory for a solo developer, but it means agent container rebuilds can transiently affect whatever the developer is looking at in a browser at that moment — worth deciding on purpose rather than by default if that's not acceptable.

**Shared-resource concurrency is lock-serialized, not capacity-planned.** Multiple runner backends (claude/cursor/codex/gemini) are designed to work WOs in parallel, but they share one Docker daemon and one physical container set per service (`COMPOSE_PROJECT_NAME` is forced identical across every worktree so containers aren't duplicated). `quality_gate.py` holds a global file lock around `make ci-local` and a per-service file lock around each service's `build` + `up` — both with stale-PID self-heal, so a killed process doesn't orphan the lock forever, but a lock still only converts contention into a queue, not into more capacity. Under real concurrent load, WOs waiting on either lock can still time out and exhaust their retries (this happened to four WOs in one day before the locks existed). If WO volume grows, the actual fix is deciding a concurrency ceiling on purpose — cap how many container-touching WOs run at once, or invest in per-worktree isolation (distinct ports/project names) so two WOs never contend for the same container at all.
