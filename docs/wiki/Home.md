...
The agentic factory is a Docker-based system that orchestrates AI agents — Claude, Cursor, Codex, Gemini — to implement software work orders autonomously. You describe a feature, capability, or change to the PM assistant in plain language. The PM drafts a structured work order spec, an agent claims it, writes the code, runs CI, gets a peer review from another model, and opens a PR. For low-risk work, the PR merges itself.
