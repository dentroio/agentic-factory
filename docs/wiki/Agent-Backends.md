...
The factory supports five AI backends for executing WOs **against the product repo** (`GITHUB_REPO`). Four are subscription CLI tools on your host. One calls the Anthropic API from Docker. Enable only the ones you use — **Settings → Agents → LLM Providers**.
