...
- **GitHub token** — fine-grained PAT (`github_pat_...`) on the **product** repo (`GITHUB_REPO`) and this engine: Contents, Pull requests, Issues, Actions. No `gist`. Classic `ghp_` / GitHub CLI `gho_` tokens are rejected. Required for everything. The repo the factory builds is set at `make agent-setup` / restart (`GITHUB_REPO`), not from this page.
